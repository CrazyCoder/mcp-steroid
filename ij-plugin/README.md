...
- Installing the plugin and connecting an agent: [repository README → Install](../README.md#install)
...
through a file. It is the settings-page twin of `devrig install config`, and both are built in
`:devrig-common` from the same launcher-path policy devrig registers with, so what you copy cannot drift
from what `devrig install <agent>` writes. Shown only once devrig is installed — before that, the snippet
would name a launcher that is not there.
...
could each start their own multi-hundred-megabyte download of the same build. So: yield while another process holds a
...
The install is a multi-hundred-megabyte download, so the installer's own output drives the IDE's progress
bar instead of a static label: the phase as text, and a real fraction from the bytes staged under
...
A failure writes the reason to `~/.mcp-steroid/markers/bootstrap-install.failed` — the marker the Claude
plugin's install wrapper writes on the agent side; its readers (`/devrig:status`, the agent's
SessionStart hook) arrive with the unmerged `claude-plugin` branch, and writing the same marker here
means an IDE-side failure will be visible to them too. A successful install clears it.
...
| The registration itself, its `--check` doctor and drift exit code (devrig side) | `:npx-kt` `devrig/InstallCommand.kt` |
